...
def load_images_as_tensor(path='data/truck', interval=1, PIXEL_LIMIT=255000, return_meta=False):
    """
    Loads images from a directory or video, resizes them to a uniform size,
    then converts and stacks them into a single [N, 3, H, W] PyTorch tensor.

    Args:
        path (str): Directory, video, or single image path.
        interval (int): Sampling stride when multiple frames are present.
        PIXEL_LIMIT (int): Upper bound on total pixels per frame after resize.
        return_meta (bool): When True, also return bookkeeping metadata that
            records the original frame sizes and resolved file paths.

    Returns:
        torch.Tensor or Tuple[torch.Tensor, dict]: If ``return_meta`` is False the
        behaviour remains unchanged and only the stacked tensor is returned. When
        True, a tuple ``(tensor, metadata)`` is produced.
    """
    sources = []
    paths_meta = []
    original_sizes = []
    ...
    if osp.isdir(path):
        ...
        for i in range(0, len(filenames), interval):
            ...
            try:
                ...
                sources.append(img)
                if return_meta:
                    paths_meta.append(osp.realpath(img_path))
        ...
        while True:
            ...
            if frame_idx % interval == 0:
                ...
                sources.append(img)
                if return_meta:
                    paths_meta.append(f"{osp.realpath(path)}#frame_{frame_idx:06d}")
                    original_sizes.append((img.height, img.width))
            frame_idx += 1
        cap.release()
        ...
        try:
            ...
            sources.append(img)
            if return_meta:
                paths_meta.append(osp.realpath(path))
                original_sizes.append((img.height, img.width))
        except Exception as e:
            raise IOError(f"Cannot open image file: {path}, error: {e}")
    else:
        raise ValueError(f"Unsupported path. Must be a directory, a .mp4 file, or an image file (.png, .jpg, .jpeg): {path}")
